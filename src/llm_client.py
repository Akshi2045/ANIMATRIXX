...
def generate_manim_code(user_instruction: str) -> str:
    """Sends a query to Gemini to write Python code using the Manim library with strict guardrails."""
    system_instruction = (
        ...
        "1. NEVER initialize completely empty text objects like Text(\"\") or Tex(\"\") and then position them using next_to() or align_to(). This triggers a fatal IndexError. Always use at least a space, like Text(\" \"), or a visible placeholder string.\n"
        ...
        "4. Ensure all imported modules and custom classes are fully defined within this single file.\n"
        ...
        "7. NEVER compare Manim direction vectors (like UP, DOWN, LEFT, RIGHT) directly using Python comparison operators (e.g., direction == DOWN). Since these are numpy arrays, this raises a ValueError. Instead, check elements or use 'all(direction == DOWN)' or 'np.array_equal(direction, DOWN)'.\n"
        "8. NEVER write new text/explanation Mobjects directly on top of existing ones. When updating on-screen texts, use 'Transform' or explicitly 'FadeOut' the old text first. Place status text at the bottom edge using 'to_edge(DOWN, buff=0.5)' to avoid spatial collisions.\n"
        "9. Keep horizontal element layouts within screen boundaries (maximum width of 14.2 units). Scale list elements appropriately and keep horizontal spacing buffers small (buff=0.1 to 0.2) to ensure nothing is clipped at the left/right screen edges."
    )
...
def extract_python_code(raw_response: str) -> str:
    """Parses out code block delimiters like ```python and returns only clean Python code."""
